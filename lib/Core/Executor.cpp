...
std::ofstream *CF = NULL;
Instruction *lastStore = NULL;
std::set<ExecutionState*> terminatedStates;

std::vector<ref<Expr> > entryFunctionArgs;
...
// (source line of a store inst, source line of a load inst (use))
std::set<std::pair<std::string, std::string> >  store_load;
std::set<std::pair<std::string, std::string> >  load_load;
std::set<std::pair<std::string, std::string> >  load_load_pointer;
std::set<std::pair<std::string, std::string> >  load_store;
std::set<std::pair<std::string, std::string> >  store_store;
std::set<std::pair<std::string, std::string> >  store_load_wcontext;
std::set<std::pair<std::string, std::string> >  load_load_wcontext;
std::set<std::pair<std::string, std::string> >  load_load_pointer_wcontext;
std::set<std::pair<std::string, std::string> >  load_store_wcontext;
std::set<std::pair<std::string, std::string> >  store_store_wcontext;

// ExecutionState -> MemoryObject -> Most Recent Load/Store
std::map<long, std::map<long, KInstruction*> > memObjLoad;
std::map<long, std::map<long, KInstruction*> > memObjLoadPointer;
std::map<long, std::map<long, KInstruction*> > memObjStore;
std::map<long, std::map<long, std::string> > memObjLoadwContext;
std::map<long, std::map<long, std::string> > memObjLoadPointerwContext;
std::map<long, std::map<long, std::string> > memObjStorewContext;

// moving to state
//std::map<int, ref<Expr> > state.valueMap;
//std::map<ref<Expr>, long> state.memobjMap;
...
bool assertionBasedChecking = false;

// helps consider branches on IPP nodes that execute globally visible actions
std::map<long, bool> watchOnGlobal;


std::set<std::string> targetFuncNames, receiveFuncNames, sendFuncNames;
std::ofstream *prf = NULL;
std::set<KInstruction *> priorityset;

bool checkGloballyVisible(ExecutionState &state, Instruction *ki, 
                          const MemoryObject *mo);

// weakest precondition (State, instruction) -> Symbolic Constraint	
// should not be copied within the ExecutionState copy constructor!
// std::map<long, std::map<long, WPState*> > wpMap;

// weakest precondition State -> Symbolic Constraint 
// for children of ippNodes (the branching is not due to an instruction!)
// should not be copied within the ExecutionState copy constructor!
// std::map<long, WPState*> ippwpMap;

void computeAndSetWP(ExecutionState &prevstate, ExecutionState &nextState, 
                     KInstruction *ki, std::vector<ref<Expr> > args);

cl::opt<bool>
RememberIP("remember-ip",
           cl::desc("Remember IPs used for interleaving to avoid rescheduling\n"),
           cl::init(false));

cl::opt<std::string>
PrioritizationStatsFile("prioritization-stats-file",
                        cl::desc("Prioritization stats file\n"));

cl::opt<bool>
PrioritizePaths("prioritize-paths",
                cl::desc("Prioritize paths based on custom criteria\n"),
                cl::init(false));

cl::opt<std::string>
TargetsFile("targets-file", 
            cl::desc("Name of the file that includes target function names\n"));

cl::opt<std::string>
ReceivesFile("receives-file", 
            cl::desc("Name of the file that includes receive function names\n"));
   
cl::opt<std::string>
SendsFile("sends-file", 
            cl::desc("Name of the file that includes send function names\n"));

cl::opt<bool>
UseConcreteArgs("use-concrete-args", 
                cl::desc("Use concrete arguments\n"),
                cl::init(false));

cl::opt<unsigned>
MaxPathSize("max-path-size", 
            cl::desc("Maximum number of instructions to be executed on a path in multithreaded mode (asyncMode)"),
            cl::init(100000));

cl::opt<unsigned>
MaxSelStates("max-sel-states",
             cl::desc("Maximum number of states to analyze to infer interleaving points (default 50)\n"),
             cl::init(50));

cl::opt<unsigned>
MaxACTime("max-ac-time",
          cl::desc("Maximum time for assertion based checking (default 1800)\n"),
          cl::init(1800));  

cl::opt<unsigned>
MaxInferTime("max-infer-time",
             cl::desc("Maximum time for the inference step time in seconds (default 1800) \n"),
             cl::init(1800)); 

cl::opt<unsigned>
MaxSelectTime("max-select-time",
             cl::desc("Maximum time for each of the selective step time in seconds (default 1800)\n"),
             cl::init(1800)); 

cl::opt<bool>
PartitionInferencePoints("partition-infer-points",
          cl::desc("Apply inference points one partition at a time\n"),
          cl::init(false));

cl::opt<unsigned>
MaxForksInference("max-forks-for-inference",
          cl::desc("Maximum number of forks for the inference step for selective mode\n"),
          cl::init(10));

cl::opt<bool>
TryIndivInferencePoints("apply-inferpoints-indiv",
                cl::desc("Applies each inference point individually (default false)\n"),
                cl::init(false));       

cl::opt<bool>
TryInferencePointPairs("apply-inferpoints-pairs",
                cl::desc("Applies inference point pairs (default false)\n"),
                cl::init(false)); 


cl::opt<std::string>
TargetFunctions("target-function-list", 
       cl::desc("Name of the file that contains target function names\n")); 

cl::opt<std::string>
ConcurrencyInfo("conc-info",
   cl::desc("The file name that records statistics about concurrency analysis\n")); 
cl::opt<std::string>
InterleavingPointsInfo("interleaving-point-info",
   cl::desc("The file name that records the interleaving points and partisions\n"));

// 0 is selective mode
// 1 is branch-level interleaving
// 2 is instruction-level interleaving
// 3 is assertion-based checking
cl::opt<unsigned>
SchedulingMode("schedule-mode",
               cl::desc("The mode of scheduling threads: 0 selective, 1 branch level, 2 instruction level, 3 assertion-based cheking \n"),
               cl::init(0));

cl::opt<unsigned>
NumExploreSelSteps("num-explore-sel",
                     cl::desc("Number of steps to schedule using inferred interleaving points\n"),
                     cl::init(1));
...
extern std::string getSourceWithContextThread(ExecutionState &state, KInstruction *ki, 
                                              bool next);
std::string getSourceWithContext(ExecutionState &state, KInstruction *ki, bool next=false);
...
std::string getStackTrace(ExecutionState &state, bool next=false);
...
void readTargetFunctions(std::string fname) {
   std::fstream ifs(fname, std::fstream::in);
   if (ifs.is_open()) {
      std::string line;
      while (std::getline(ifs,line)) {
            line = ltrim(rtrim(line));   
            targetFunctions.insert(line);
      }
      ifs.close();
   }
}

bool isATargetFunction(std::string f) {
   return (targetFunctions.find(f) != targetFunctions.end());
}

void updatePathWithValueMemObjectMaps(ExecutionState &state, int index);
...
void recordMostRecent(ExecutionState &state, const MemoryObject *mo, 
                      KInstruction *kinst) {
  std::map<long, KInstruction*> mapL = memObjLoad[(long)&state];
  //std::map<long, KInstruction*> mapLP = memObjLoadPointer[(long)&state];
  std::map<long, KInstruction*> mapS = memObjStore[(long)&state];
  std::map<long, std::string> mapLwC = memObjLoadwContext[(long)&state];
  //std::map<long, std::string> mapLPwC = memObjLoadPointerwContext[(long)&st$
  std::map<long, std::string> mapSwC = memObjStorewContext[(long)&state];  
  std::string ctx = getStackTrace(state);
  std::string sl = kinst->getSourceLocation();
  if (kinst->inst->getOpcode() == Instruction::Load) {
     // record dependencies first
     if (mapL.find((long)mo) != mapL.end()) {
        std::string l1 = mapL[(long)mo]->getSourceLocation();
        load_load.insert(std::make_pair(l1,sl));
        llvm::errs() << "load-load relationship\n";
     }     
     if (mapLwC.find((long)mo) != mapLwC.end()) {
        std::string l1 = mapLwC[(long)mo];
        load_load_wcontext.insert(std::make_pair(l1,ctx));
        llvm::errs() << "load-load relationship with context \n";
     }       
     if (mapS.find((long)mo) != mapS.end()) {
        std::string s = mapS[(long)mo]->getSourceLocation();
        store_load.insert(std::make_pair(s,sl));
        llvm::errs() << "store-load relationship\n";
     }
     if (mapSwC.find((long)mo) != mapSwC.end()) {
        std::string s = mapSwC[(long)mo];
        store_load_wcontext.insert(std::make_pair(s,ctx));
        llvm::errs() << "store-load relationship with context \n";
     }
     mapL[(long)mo] = kinst;
     memObjLoad[(long)&state] = mapL;
     mapLwC[(long)mo] = ctx;
     memObjLoadwContext[(long)&state] = mapLwC;
     llvm::errs() << "recorded most recent load\n";
  }
  else if (kinst->inst->getOpcode() == Instruction::Store) {
     if (mapL.find((long)mo) != mapL.end()) {
        std::string l = mapL[(long)mo]->getSourceLocation();
        load_store.insert(std::make_pair(l,sl));
        llvm::errs() << "load-store relationship\n";
     }
     if (mapLwC.find((long)mo) != mapLwC.end()) {
        std::string l = mapLwC[(long)mo];
        load_store_wcontext.insert(std::make_pair(l,ctx));
        llvm::errs() << "load-store relationship with context \n";
     }  
     if (mapS.find((long)mo) != mapS.end()) {
        std::string s1 = mapS[(long)mo]->getSourceLocation();
        store_store.insert(std::make_pair(s1,sl));
        llvm::errs() << "store-store relationship\n";
     }
     if (mapSwC.find((long)mo) != mapSwC.end()) {
        std::string s1 = mapSwC[(long)mo];
        store_store_wcontext.insert(std::make_pair(s1,ctx));
        llvm::errs() << "store-store relationship\n";
     }
     mapS[(long)mo] = kinst;
     memObjStore[(long)&state] = mapS;
     mapSwC[(long)mo] = ctx ;
     memObjStorewContext[(long)&state] = mapSwC;
     llvm::errs() << "recorded most recent store\n";
   }
}
...
void Executor::generateValuedInstruction(ExecutionState &state, 
                                         KInstruction *ki) {
   if (!ki) return;
   if (!ki->inst) return;
   // executing a return instruction may pop the stack frame so 
   // handled immediately after executing it
...
bool getDataConstraint(std::string tname, unsigned offset, 
     ...
     llvm::errs() << "Checking data constraint for " << tname << "\n";
     for(auto mi : dataConstraintMap) {
        llvm::errs() << "data const type " << mi.first << "\n";
        if (mi.first.find(tname) != std::string::npos || 
               tname.find(mi.first) != std::string::npos) {
           llvm::errs() << "Data constraint for " << mi.first << " for " << tname << "\n";
...
void applyCommandlineSpecInitializations(ExecutionState &state, TimingSolver *solver, 
   ...
   llvm::errs() << "Checking function pointer initializations for type: " << tname << " in:\n";
   for(auto lfp : lazyFuncPtrInitializersUsingFields)
       llvm::errs() << "entry " << lfp.first << "\n";
...
void applyCommandlineSpecInitializationsToArray(ExecutionState &state, TimingSolver *solver, 
   ...
   llvm::errs() << "Applying commandline spec initializers \n";
   StructType *st = dyn_cast<StructType>(mo->baseType);
   if (!st) return;
   DataLayout *targetData = kmoduleExt->targetData;
   for(unsigned i = 0; i<count; i++) {
...
std::string getStackTrace(ExecutionState &state, bool next) {
    std::string MsgString;
    llvm::raw_string_ostream msg(MsgString);
    state.dumpStack(msg, next);
    return msg.str();
}

std::string removeArgs(std::string str) {
  bool changed = false;
  do {
     int pos1 = str.find("(");
     ...
     for(; index < length; index++) {
         if (str[index] == ')') {
            pos2 = index;
            str[index] = ']'; 
            //llvm::errs() << "Erasing starting at " << pos1+1
            //             << " and " <<  pos2 - pos1 - 1 << " chars!\n";
            str = str.erase(pos1 + 1, pos2 - pos1 - 1);
            changed = true;
            length = str.length();
            break;
         }
...
std::string getSourceWithContext(ExecutionState &state, KInstruction *ki, 
                                 bool next) {
   std::string swctx;
   swctx = getStackTrace(state, next);
   llvm::errs() << "stack trace=" << swctx << "\n";
   swctx = swctx + ki->getSourceLocation();
   //llvm::errs() << "before remove args: " << swctx << "\n";
   std::string res = removeArgs(swctx);
   //llvm::errs() << "after remove args: " << res << "\n";
   return res;
}
...
void Executor::initializeGlobals(ExecutionState &state) {
  ...
  for (Module::const_global_iterator i = m->global_begin(),
	...
	size = kmodule->targetData->getTypeStoreSize(ty);
      ...
      ref<Expr> moAddr = mo->getBaseExpr();
      ConstantExpr *moA = dyn_cast<ConstantExpr>(moAddr);
      state.globals.insert(moA->getZExtValue());
      ...
      if (size) {
        ...
        if (!addr) {
          ...
          if (!lazyInit)
             ...
             llvm::errs() << "Symbolizing global variable: " 
                          << i->getName().data() << "\n"; 
         ...
         llvm::errs() << "Initializing global variable with concrete val: " 
                          << i->getName().data() << "\n"; 
           /* SYSREL EXTENSION */
      ...
      //llvm::errs() << "Global address " << mo->getBaseExpr() 
      //             << " width " << mo->getBaseExpr()->getWidth() << "\n";
      ref<Expr> mob = mo->getBaseExpr();
      ConstantExpr *moA = dyn_cast<ConstantExpr>(mob);
      state.globals.insert(moA->getZExtValue());
  ...
  for (Module::const_global_iterator i = m->global_begin(),
    ...
    if (i->hasInitializer()) {
      ...
      ConstantExpr *moA = dyn_cast<ConstantExpr>(mob);
      state.globals.insert(moA->getZExtValue());
      //llvm::errs() << "Global var with initializer: " << i->getName().data() << "\n";
...
void Executor::branch(ExecutionState &state, 
  ...
  if (MaxForks!=~0u && stats::forks >= MaxForks) {
    ...
    for (unsigned i=1; i<N; ++i) {
      ...
      llvm::errs() << "State " << ns << " being added\n";
      ...
      llvm::errs() << "Tree updated with state " << ns << "\n";
    }
  }
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  if (res==Solver::True) {
    ...
    if (assertionBasedChecking) {
       ...
       args.push_back(ConstantExpr::create(0,64));
    ...
    if (assertionBasedChecking) {
       std::vector<ref<Expr> > args;
       args.push_back(Expr::createIsZero(condition));
       args.push_back(ConstantExpr::create(1,64));
       setWPState(current, current, current.prevPC->inst, args, NULL);
    }
    ...
    llvm::errs() << "State " << falseState << " being added\n";
    ...
    llvm::errs() << "Tree updated with state " << falseState << "\n";
    llvm::errs() << "Tree updated with state " << trueState << "\n";
    ...
    if (assertionBasedChecking) {
       ...
       argsT.push_back(ConstantExpr::create(0,64));
       argsF.push_back(falseState->lastConstraint);
       argsF.push_back(ConstantExpr::create(1,64));
...
bool Executor::handledModeledCall(ExecutionState &state, 
                           KInstruction *ki,
                           Function *f,
     ...
     if (functionModeledBy.find(removeDotSuffix(f->getName())) != functionModeledBy.end()) {
        llvm::errs() << "WARNING: Using " << functionModeledBy[removeDotSuffix(f->getName())] 
                        << " for " << f->getName() << "\n";
        Function *modelFn = moduleHandle->getFunction(
                                          functionModeledBy[removeDotSuffix(f->getName())]);
        if (modelFn) {  
...
void Executor::executeCall(ExecutionState &state, 
  ...
  if (handledModeledCall(state, ki, f, arguments))
     return;
...
void Executor::dispatchAlternativeInstructions(ExecutionState &state, 
                                            KInstruction *ki, 
                                            KInstruction *pki, 
                          std::vector<ExecutionState*> *alt) {
 if ((state.rtid == -1 && !state.activeInferenceForToBeExecuted) || 
      (state.rtid != -1 && !state.threads[state.rtid].activeInferenceForToBeExecuted)) {
    llvm::errs() << "not active so branching skipped\n";
    state.dumpThreadStates();
    return;
 }
  if (ki != pki) {
     Function *f = ki->inst->getParent()->getParent();
     llvm::errs() << "branching before " << (*ki->inst) 
                  << " in func " << f->getName() << "\n";
     state.dumpThreadStates();
     state.branchMT(*alt, false);
  } 
}

void Executor::dispatchAlternativeBlocks(ExecutionState &state, 
                                            KInstruction *ki, 
                                            KInstruction *pki, 
                          std::vector<ExecutionState*> *alt) {
 if ((state.rtid == -1 && !state.activeInferenceForToBeExecuted) || 
...
void Executor::dispatchAlternativeSchedules(ExecutionState &state, 
                                            KInstruction *ki, 
                                            KInstruction *pki, 
                    std::set<std::pair<Instruction*,std::string> > *IP, 
                          std::vector<ExecutionState*> *alt) {
 llvm::errs() << " In dispatch with " << (*pki->inst) 
              << " next " << (*ki->inst) << "\n";
 if (ki != pki) { 
    if ((state.rtid == -1 && state.activeInferenceForExecuted) || 
         (state.rtid != -1 && state.threads[state.rtid].activeInferenceForExecuted))
    {
       Instruction *pi = pki->inst;
       if (pi->getOpcode() == Instruction::Call || 
          pi->getOpcode() == Instruction::Invoke) {
          CallSite cs(pi);
          Value *fp = cs.getCalledValue();
          Function *f = getTargetFunction(fp, state);
          if (f && f->getName().find("unlock") != std::string::npos) {
             std::string ctx;
             if (state.rtid == -1)
                ctx = getSourceWithContext(state,pki);
             else 
                ctx = getSourceWithContextThread(state,pki,false);
             llvm::errs() << "checking for unlock alt sched point : " << (*pi) 
                          << " using context " << ctx << "\n";
             std::pair<Instruction*,std::string> p = std::make_pair(pi,ctx);
             if (IP && alt) {
                if (IP->find(p) != IP->end()) {
                   if (RememberIP) {
                      if (!state.interleavedAt(p)) {
                        state.recordIPSchedule(p);
                        state.branchMT(*alt, true);
                      }
                      else llvm::errs() << "Skipping rescheduling\n";
                   }
                   else state.branchMT(*alt, true);
                } 
             }
          }
          else if (f && f->getName().find("pthread_create") != std::string::npos) {
                if (state.isInterleavingDelayed()) {
                   state.setInterleavingDelayed(false); 
                   state.branchMT(*alt, true);
                }
          }
       }
  ...
  if ((state.rtid == -1 && !state.activeInferenceForToBeExecuted) || 
      (state.rtid != -1 && !state.threads[state.rtid].activeInferenceForToBeExecuted))
     // this is to apply branching according to inferred scheduling points 
     // at most once at every program location
     return;
  Instruction *i = ki->inst;
  if (i->getOpcode() == Instruction::Call || 
     ...
     Function *f = getTargetFunction(fp, state);
     if (f && f->getName().find("unlock") != std::string::npos) { 
        // release ops are handled after being executed (see above)
        // acquires (blocking ops in general) are handled before being executed (see below)
        return; 
     }
  }
  std::string ctx;
  if (state.rtid == -1)
     ctx = getSourceWithContext(state,ki,true);
  else 
     ctx = getSourceWithContextThread(state,ki,true);
  llvm::errs() << "checking for alt sched point : " << (*i) 
     << " using context " << ctx << "\n";
...
void Executor::propagateWP(WPState *wps, ref<Expr> wp) {
   std::string i1 = "null";
   if (wps->ki) {
      std::string str1;
      llvm::raw_string_ostream rso(str1);
      wps->ki->print(rso);
      i1 = rso.str();
   }
   std::string i2 = "null";
   if (wps->parent->ki) {
      std::string str2;
      llvm::raw_string_ostream rso(str2);
      wps->parent->ki->print(rso);
      i2 = rso.str();
   }
   llvm::errs() << "In propagateWP, for " << wps->id << " " << i1 
                << " parent " << wps->parent->id << " " << i2 << "\n";
   WPState *parent = wps->parent;
   if (!parent) return;
   parent->numChildrenCompleted++;
   bool ippNode = (wps->ki == NULL );
   bool bppNode =  (*wps->type == "assume");
   if (wps->condition.isNull()) {
      if (*wps->type == "store") {
         llvm::errs() << "Recording wps for store\n";
         if (wps->mo->name == "") {
            wps->condition = wp;
         }
         else {
            wps->condition = rewriteExp(memory, wps->mo, wp, 
                                 (*wps->args)[0], (*wps->args)[1], (*wps->args)[2]);
            llvm::errs() << "Symbolic value in store " << (*wps->args)[2] 
                         << " rewritten as " << wps->condition << "\n";
         }
      }
      else if (*wps->type == "assume") {
           llvm::errs() << "Recording wps for assume " << (*wps->args)[0] << "\n";
           wps->condition = AndExpr::create(wp,(*wps->args)[0]);
      }   
      else if (*wps->type == "assert") {
           llvm::errs() << "Recording wps for assert " << (*wps->args)[0] << "\n";
           wps->condition = AndExpr::create(wp,(*wps->args)[0]);
      }  
      else // other or ippchild
           wps->condition = wp;
   }
   if (wps->condition.isNull())
      llvm::errs() << "Propagating condition is null\n";
   else  
      llvm::errs() << "Propagating condition " << wps->condition << "\n";
   if (ippNode) {
      llvm::errs() << "IPP child propagating\n";
      if (parent->condition.isNull())
         parent->condition =  wps->condition;
      else 
         parent->condition = AndExpr::create(parent->condition, wps->condition); 
      if (parent->children->size() == parent->numChildrenCompleted)
         propagateWP(parent, parent->condition);
   }
   else if (bppNode) {
      llvm::errs() << "BPP child is propagating\n";
      if (parent->condition.isNull())
         parent->condition =  wps->condition;
      else  
         parent->condition = OrExpr::create(parent->condition, wps->condition); 
      if (parent->children->size() == parent->numChildrenCompleted)
         propagateWP(parent, parent->condition);
   }
   else {
      llvm::errs() << "Regular WPS node propagating\n";
      propagateWP(parent, wps->condition);
   }
}

void restoreChildCount(ExecutionState &state) {
     WPState *temp = state.currentwps;
     while (*temp->type != "ippchild") {
         assert(temp->parent);
         temp = temp->parent;
     }
     assert(temp->parent);
     assert(temp->parent->children);    
     std::vector<long>::iterator it = temp->parent->children->begin(); 
     for (; it != temp->parent->children->end(); it++) {
         if ((*it) == (long)temp)
            break; 
     }
     if (it != temp->parent->children->end())
        temp->parent->children->erase(it);
     
}

void Executor::setWPState(ExecutionState &prevstate, ExecutionState &nextstate, 
                          Instruction *ki, std::vector<ref<Expr> > args, 
                          const MemoryObject *mo) {

   WPState *wps = prevstate.currentwps;
   assert(wps && "Current wps cannot be null!\n");
   WPState *nwps = new WPState();
   for(unsigned i=0; i<args.size(); i++)
      nwps->args->push_back(args[i]);
   nwps->mo = mo;
   nwps->ki = ki;
   nwps->state = (long)&nextstate;
   nwps->tid = nextstate.rtid;
   bool propagate = false;
   if (ki) {
      llvm::errs() << "Setting WPS type for " << (*ki) << "\n";
      if (ki->getOpcode() == Instruction::Store) {
         // arg[0]: offset
         // arg[1]: width
         // arg[2]: stored value
         nwps->type = new std::string("store");
      }
      else if (ki->getOpcode() == Instruction::Br) {
        // arg[0]: condition
        // arg[1]: true/false branch
        BranchInst *bi = cast<BranchInst>(ki);
        if (!bi->isUnconditional())  {
           llvm::errs() << "Setting WPS info for assume: " << args[0] << "\n";
        /*
           ConstantExpr *CE = dyn_cast<ConstantExpr>(args[1]);
           int target = -1;
           if (CE) target = CE->getZExtValue();
           // is it assert condition related branching? 
           bool assertRelated = false;
           int assertTrueTarget = -1;
           if (bi->getNumSuccessors() == 2) {
              BasicBlock *bb1 = bi->getSuccessor(0);
              for(Instruction &bi : *bb1) {
                 std::string fstr;
                 llvm::raw_string_ostream rso(fstr);
                 bi.print(rso);
                 std::string istr = rso.str();
                 if (istr.find("assert") !=  std::string::npos) {
                    assertRelated = true;
                    assertTrueTarget = 1; // reverse, fail is explicitly mentioned
                    break;
                 }
              }
              BasicBlock *bb2 = bi->getSuccessor(1);
              if (!assertRelated) {
                 for(Instruction &bi : *bb2) {
                    std::string fstr;
                    llvm::raw_string_ostream rso(fstr);
                    bi.print(rso);
                    std::string istr = rso.str();
                    if (istr.find("assert") !=  std::string::npos) {
                       assertRelated = true;
                       assertTrueTarget = 0; //reverse, fail is explicitly mentioned 
                       break;
                    }
                 }
              }
           }
           llvm::errs() << "Branch instruction " << (*ki) 
                        << "assert related ? " << assertRelated 
                        << "target " << target 
                        << "target for assert hold " << assertTrueTarget << "\n";  
           if (assertRelated) { 
              if (target == assertTrueTarget)
                 nwps->type = new std::string("assert");
              else nwps->type = new std::string("other");
           }
           else */ nwps->type = new std::string("assume");
        }
        else nwps->type = new std::string("other");
      }
      else if (ki->getOpcode() == Instruction::Call || 
            ki->getOpcode() == Instruction::Invoke) {
         // arg[0]: Function*
         // arg[1]: the first actual parameter
         CallSite cs(ki);
         Value *fp = cs.getCalledValue();
         if (fp && (fp->getName() == "assert" || fp->getName() == "__assert_fail"))
            nwps->type = new std::string("assert");
         else if (fp && (fp->getName() == "pthread_exit" || 
                       fp->getName() == "exit")) {
            nwps->type = new std::string("exit");
            nwps->condition = ConstantExpr::create(true, 1);
            propagate = true;
         }
         else nwps->type = new std::string("other");
      }
      else if (ki->getOpcode() == Instruction::Ret) {
         llvm::errs() << "Returning from " 
                      << ki->getParent()->getParent()->getName() << "\n";
         if (ki->getParent()->getParent()->getName() == entryFunc->getName() || 
             ki->getParent()->getParent()->getName() == 
                                   nextstate.getCurrentThreadEntryFunctionName()) {
            llvm::errs() << "Propagating from an exit point\n";
            nwps->type = new std::string("exit");
            nwps->condition = ConstantExpr::create(true, 1);
            propagate = true;
         }
   ...
   else nwps->type = new std::string("ippchild");
   nwps->parent = wps;
   wps->children->push_back((long)nwps);
   wps->numChildrenCompleted++;
   nextstate.currentwps = nwps;	
   if (propagate)
      propagateWP(nwps, nwps->condition);
}

bool reaches(BasicBlock *bb1, BasicBlock *bb2, std::set<BasicBlock*> &visited) {
   if (visited.find(bb1) != visited.end())
      return false;         
   visited.insert(bb1);
   if (bb1 == bb2)
      return true;
   Instruction *i = bb1->getTerminator();
   if (i->getOpcode() != Instruction::Br) {
      llvm::errs() << "Non-branch inst: " << (*i) << "\n";
      return false;
   }
   BranchInst *bi = cast<BranchInst>(i);
   for(int i=0; i<bi->getNumSuccessors(); i++)  {
      if (reaches(bi->getSuccessor(i), bb2, visited))
         return true;
   }
   return false;
}

bool reachesInMultipleWays(KInstruction *br, KInstruction *ki) {
   assert(br->inst->getOpcode() == Instruction::Br);
   BranchInst *bi = cast<BranchInst>(br->inst);
   if (bi->isUnconditional()) 
      return false;
   int reaching = 0;
   for(int i=0; i<bi->getNumSuccessors() && reaching < 2; i++) {
      BasicBlock *bb = bi->getSuccessor(i);
      std::set<BasicBlock*> v;   
      if (reaches(bb, ki->inst->getParent(), v))
         reaching++;
   }
   return (reaching >= 2);
}

bool Executor::isInScope(ExecutionState &state, KInstruction *target, KInstruction *br) {
  if (br->inst->getOpcode() != Instruction::Br)
      return false;
  llvm::errs() << "branch inst: " << (*br->inst) 
               << "function: " << ((Function*)br->inst->getParent()->getParent())->getName() << "\n";
  llvm::errs() << "target inst: " << (*target->inst)  
               << "function: " << ((Function*)target->inst->getParent()->getParent())->getName() << "\n";

  if (((Function*)target->inst->getParent()->getParent())->getName() == 
      ((Function*)br->inst->getParent()->getParent())->getName()) {
     return !reachesInMultipleWays(br, target);    
  }
  else {
     int tindex = -1;
     for(int i=state.KPath.size()-1; i>=0; i--) {
        if (state.KPath[i] == target) {
           tindex = i;
           break;
        }
     }
     ...
     for(int i=tindex-1; i>=0; i--) {
        if (state.KPath[i]->inst->getOpcode() == Instruction::Invoke ||
            state.KPath[i]->inst->getOpcode() == Instruction::Call) {
           CallSite cs(state.KPath[i]->inst);
           Value *fp = cs.getCalledValue();
           Function *f = getTargetFunction(fp, state);
           std::string cfname = f->getName();
           std::string tfname = ((Function*)state.KPath[tindex]->inst->getParent()->getParent())->getName(); 
           llvm::errs() << "target fname=" << tfname << " candidate fname=" << cfname << "\n";
           if (tfname == cfname) {
              csindex = i;
              break; 
           }
     ...
     if (csindex == -1)
        return false;
     return isInScope(state, state.KPath[csindex], br);
  }
}


bool isATargetName(std::string name) {
  if (TargetsFile != "") {
     if (targetFuncNames.size() == 0) {
        std::fstream tf(TargetsFile.c_str(), std::fstream::in);
        if (tf.is_open()) {
           std::string line;
           while(std::getline(tf,line)) { 
                targetFuncNames.insert(ltrim(rtrim(line)));
           } 
           tf.close();
        }
     }
     for(auto tf : targetFuncNames) {
        if (name.find(tf) != std::string::npos)
           return true;
     }
  }
  return false;
...
bool Executor::isCallATarget(ExecutionState &state, KInstruction *ki) {
  if (ki->inst->getOpcode() == Instruction::Invoke ||
      ki->inst->getOpcode() == Instruction::Call) {
     CallSite cs(ki->inst);
     Value *fp = cs.getCalledValue();
     Function *f = getTargetFunction(fp, state);
     return (f ? isATargetName(f->getName()) : false);
  }
  return false;
}

bool isAReceiveName(std::string name) {
  if (ReceivesFile != "") {
     if (receiveFuncNames.size() == 0) {
        std::fstream rf(ReceivesFile.c_str(), std::fstream::in);
        if (rf.is_open()) {
           std::string line;
           while(std::getline(rf,line)) { 
                receiveFuncNames.insert(ltrim(rtrim(line)));
           } 
           rf.close();
        }
     }
     for(auto rf : receiveFuncNames) {
...
bool Executor::isAReceiveCall(ExecutionState &state, KInstruction *ki) {
  if (ki->inst->getOpcode() == Instruction::Invoke ||
     ki->inst->getOpcode() == Instruction::Call) {
     CallSite cs(ki->inst);
     Value *fp = cs.getCalledValue();
     Function *f = getTargetFunction(fp, state);
     return isAReceiveName(f->getName());    
  }
  return false;
}

bool isASendName(std::string fname) {
  if (SendsFile != "") {
     if (sendFuncNames.size() == 0) {
        std::fstream sf(SendsFile.c_str(), std::fstream::in);
        if (sf.is_open()) {
           std::string line;
           while(std::getline(sf,line)) { 
                sendFuncNames.insert(ltrim(rtrim(line)));
           } 
           sf.close();
        }
     }
     for(auto sf : sendFuncNames) {
        if (fname.find(sf) != std::string::npos) 
           return true;
     }
  }
  return false;
...
bool Executor::isASendCall(ExecutionState &state, KInstruction *ki) {
 if (ki->inst->getOpcode() == Instruction::Invoke ||
     ki->inst->getOpcode() == Instruction::Call) {
    CallSite cs(ki->inst);
    Value *fp = cs.getCalledValue();
    Function *f = getTargetFunction(fp, state);
    return isASendName(f->getName());
  }
  return false;
}

bool isTaintedFromAReceive(const MemoryObject *mo) {
  if (mo == NULL)
     return false;
  return isAReceiveName(mo->name);
}

bool isMemoryAccessATarget(ExecutionState &state, KInstruction *ki) {
  if (ki->inst->getOpcode() == Instruction::Load || 
      ki->inst->getOpcode() == Instruction::Store) {
     if (!state.lastAccessedAddr.isNull()) {
        klee::ConstantExpr *CE = dyn_cast<klee::ConstantExpr>(state.lastAccessedAddr);
        if (!CE) return true;
     }
     return isTaintedFromAReceive(state.lastAccessedMem);
  }
  return false;
}

bool Executor::isATargetInstruction(ExecutionState &state, KInstruction *ki) {
  switch (ki->inst->getOpcode()) {
    case Instruction::Load: 
    case Instruction::Store: {
       return isMemoryAccessATarget(state, ki); 
    }
    case Instruction::Invoke:
    case Instruction::Call: {
       return isCallATarget(state, ki);
    }
    default: return false;
  }
}

void initalizeForPriorityMode() {
  // trigger population of the relevant sets with mock parameters
  isASendName("temp");
  isAReceiveName("temp");
  for(auto s : sendFuncNames) {
     APIHandler::addIgnoreHandler(s);
  }
  for(auto r : receiveFuncNames) {
     APIHandler::addIgnoreHandler(r);
  }
}

void Executor::checkTargetUpdatePrioritized(ExecutionState &state, KInstruction *ki) {

   // Check if ki is a target instruction
   if (isATargetInstruction(state, ki)) {
      llvm::errs() << "Target instruction: " << (*ki->inst) << "\n";
      (*prf) << "Target location: \n"; 
      (*prf) << ki->getSourceLocation() << "\n";
      prf->flush();
      //compute priority set
      // std::set<KInstruction *> priorityset;
      for(int i=0; i<state.KPath.size(); i++) {
         if (state.KPath[i]->inst->getOpcode() == Instruction::Br && !isInScope(state, ki, state.KPath[i])) {
            llvm::errs() << "\t not in scope of " << (*state.KPath[i]->inst) << "\n";
            priorityset.insert(state.KPath[i]);
         }
      }
      // Check every state in states and pause those that branched from unprioritized ones
      std::set<ExecutionState*> tobestashed;
      for(auto s : states) {
         llvm::errs() << "state prevPC : " << (*s->prevPC->inst) << " pc : " << (*s->pc->inst) << "\n";
         if (s != &state && s->prevPC->inst->getOpcode() == Instruction::Br &&
             priorityset.find(s->prevPC) == priorityset.end()) {
            llvm::errs() << "Pausing state at inst " << (*s->prevPC->inst) << "\n";
            (*prf) << "Pausing state at inst " << s->prevPC->getSourceLocation() << "\n"; 
            prf->flush();
            tobestashed.insert(s);
         }
      }
      for(auto p : tobestashed) {
         states.erase(p);
         stashedStates.insert(p);
         searcher->stashState(p);
      }
      (*prf) << "Num states=" << states.size() << "\n";
      (*prf) << "Num stashed states=" << stashedStates.size() << "\n";
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki, 
                    std::set<std::pair<Instruction*,std::string> > *IP, 
                                  std::vector<ExecutionState*> *alt) {
    if (asyncMode && state.Path.size() > MaxPathSize) {
  ...
  Instruction *i = ki->inst;
  std::string ctx = getSourceWithContext(state,ki);
  std::pair<Instruction*,std::string> p = std::make_pair(i,ctx);
  /* if (IP && alt) {
     if (IP->find(p) != IP->end())
        state.branchMT(*alt);
  ...
  case Instruction::Ret: {
    ...
    if (!isVoidReturn) {
      ...
      llvm::errs() << "return value " << result << " for " << ri->getParent()->getParent()->getName() << "\n";
    ...
    if (state.stack.size() <= 1) {
      ...
      llvm::errs() << "has lcm? " << state.hasLCM() << "\n";
      ...
      bool isSExt = cs.paramHasAttr(0, llvm::Attribute::SExt);
          ...
          llvm::errs() << "Storing return value for inst " << (*ki->inst) << "\n";
        }
      } else {
      ...
      if (loopBound && !lbe) {
         ...
         if (branches.first && branches.second) {
	       ...
	       terminateStateOnError(state, "Memory error", Ptr, NULL, 
            ...
            state.updateLikelyRelevantBranches(state.Path.size()-1, 1);
            llvm::errs() << "Recording likely rel branch (" 
                         << state.Path.size()-1 << ",1)\n" ;
         }
         else if (branches.second) {
            state.updateLikelyRelevantBranches(state.Path.size()-1, 0);
            llvm::errs() << "Recording likely rel branch (" 
                         << state.Path.size()-1 << ",0)\n" ;
         }
      }
  ...
  case Instruction::Call: {
    ...
    llvm::errs() << "Function call instr: " << rso.str() << "\n";

    if (f && f->getName() == "abort") {
       state.terminateThread(state.rtid);
       llvm::errs() << "Thread " << state.rtid << " terminating due to abort\n";
       return;
    }
    ...
    for (unsigned j=0; j<numArgs; ++j) {
      ...
      state.valueMap[j] = eval(ki, j+1, state).value;
      ...
      do {
        ...
        if (res.first) {
          ...
          if (assertionBasedChecking) {
            ...
            updatePathWithValueMemObjectMaps(*res.first,res.first->Path.size()-1);
...
void Executor::updateStatesMT(ExecutionState *current,
                              std::vector<ExecutionState*> altsch) {
   ExecutionState *es = current;

   for(unsigned i=0; i<altsch.size(); i++) {
      llvm::errs() << "State " << altsch[i] << " being added\n";
      addedStates.push_back(altsch[i]);
   }

  for(unsigned i=0; i<altsch.size(); i++) {
     es->ptreeNode->data = 0;
     std::pair<PTree::Node*,PTree::Node*> res = 
           processTree->split(es->ptreeNode, altsch[i], es);
     altsch[i]->ptreeNode = res.first;
     es->ptreeNode = res.second;
     es = altsch[i];
     llvm::errs() << "Tree updated with state " << altsch[i] << "\n";

  }

  if (searcher) {
    llvm::errs() << "Updating searcher for state " << current << "\n";
    searcher->update(current, addedStates, removedStates);
    //for(unsigned i=0; i<altsch.size(); i++) 
    //   searcher->update(altsch[i], addedStates, removedStates);
...
void Executor::updateStates(ExecutionState *current) {
  if (searcher) {
    llvm::errs() << "Updating searcher for state " << current << "\n";
    llvm::errs() << "added: " << addedStates.size() 
                 << "removed: " << removedStates.size() << "\n";
    searcher->update(current, addedStates, removedStates);
    llvm::errs() << "cont: " << continuedStates.size() 
                 << "paused: " << pausedStates.size() << "\n";
    searcher->update(nullptr, continuedStates, pausedStates);
    pausedStates.clear();
    continuedStates.clear();
    llvm::errs() << "Active states : " << states.size() << "\n";
  }
...
void Executor::doDumpStates() {
  ...
  if (asyncMode) {
     llvm::errs() << "Early states: \n";
     for (const auto &state : states) {
         llvm::errs() << state->getThreadSchedule() << "\n";
     }
     llvm::errs() << "Terminated states: \n";
     for (const auto &state : terminatedStates) {
         llvm::errs() << state->getThreadSchedule() << "\n";
     }
  }
...
void resetActiveInference(ExecutionState &state) {
   // now that some instruction has been executed, activeInference can be reset
   if (state.rtid == -1) {
      state.activeInferenceForExecuted = true; 
      state.activeInferenceForToBeExecuted = true; 
   }
   else {
      Async &a = state.threads[state.rtid];
      a.activeInferenceForExecuted = true; 
      a.activeInferenceForToBeExecuted = true; 
   }

}

void updatePathWithValueMemObjectMaps(ExecutionState &state, int index) {
   state.PathMemObject[index] = state.memobjMap;
   state.PathValueObject[index] = state.valueMap;
}

bool checkGloballyVisible(ExecutionState &state, Instruction *inst, 
                          const MemoryObject *mo=NULL) {
  llvm::errs() << "Checking if instruction is globally visible\n";
  if (inst->getOpcode() == Instruction::Store || 
         inst->getOpcode() == Instruction::Load) {
     assert(mo && "NULL memory object in load/store global visibility check!\n");
     return mo->isGlobal;
  }
  if (inst->getOpcode() == Instruction::Call || 
      ...
      Value *fp = cs.getCalledValue(); 
      if (fp && fp->getName().find("pthread") != std::string::npos)
         return true;
  }
  return false;
}
...
void Executor::exploreAssertionBased(ExecutionState &initialState) {   
   initRun(initialState);
   double elapsedACTime = 0;
   double acStartTime = statsTracker->elapsed();
   while (!states.empty() && !haltExecution && elapsedACTime < MaxACTime) {
       llvm::errs() << "Assertion based checking elapsed time=" 
                    << elapsedACTime << "\n";
       ExecutionState &state = searcher->selectState();
       successorsPaths->erase(successorsPaths->begin(), 
                              successorsPaths->end());
       successorsPaths->insert(&state);

       int tid = -2; 
       if (state.isEnabled(state.rtid)) 
          tid = state.rtid;
       else {
          tid = state.scheduleAsync(memory);
       } 

       std::vector<ExecutionState*> altschedules;
       if ((state.rtid == -1 && state.activeInferenceForToBeExecuted) || 
          (state.rtid >= 0 && 
           state.threads[state.rtid].activeInferenceForToBeExecuted))
          state.branchMT(altschedules, false);
       std::vector<ref<Expr> > eargsv;
       llvm::errs() << "Creating alternative schedules of size " 
                    <<  altschedules.size() << "\n";
       for(unsigned i = 0; i < altschedules.size(); i++) {

          // set a flag for each forked state so that if not executing a globally
          // visible instruction, state gets cancelled
          watchOnGlobal[(long)altschedules[i]] = true;

          setWPState(state, (*altschedules[i]), NULL, eargsv, NULL);
       }
       if (altschedules.size())
          setWPState(state, state, NULL, eargsv, NULL);

       if (tid < -1) {
          if (!state.allTerminated())
             terminateStateOnError(state, "Deadlock", Deadlock, "deadlock");
          else
             terminateStateOnExit(state);
          //break;
       }
       else {
           KInstruction *kia = NULL;
          if (tid == -1) { // main thread
             kia = state.pc;
             llvm::errs() << "Executing in main: " 
                          << (*kia->inst) << "\n";
             stepInstruction(state);
             state.valueMap.clear();
             state.memobjMap.clear();
             executeInstruction(state, kia, NULL, NULL);
             resetActiveInference(state);
          }
          else {
             kia = state.threads[tid].pc;
             llvm::errs() << "Executing in thread: " << tid << " " 
                          << (*kia->inst) << "\n";
             stepInstructionThread(state, tid);
             state.valueMap.clear();
             state.memobjMap.clear();
             executeInstructionThread(state, kia, tid, NULL, NULL);
             resetActiveInference(state);
          }
          bool term = false;
          if (watchOnGlobal.find((long)&state) != watchOnGlobal.end()) {
             llvm::errs() << "After execution, checking watch on global status " 
                          << watchOnGlobal[(long)&state] << "\n";
             if (watchOnGlobal[(long)&state]) {
                terminateStateOnCancel(state);
                term = true; 
             }
          }
       ...
       updateStatesMT(&state, altschedules);       
       elapsedACTime = statsTracker->elapsed() - acStartTime;
    }
}

void Executor::exploreInfer(ExecutionState &initialState,
                      std::set<std::pair<Instruction*,std::string> >&IP) {

   initRun(initialState);
   double elapsedInferTime = 0;
   double inferStartTime = statsTracker->elapsed();
   while (!states.empty() && !haltExecution && 
           stats::forks < MaxForksInference && elapsedInferTime < MaxInferTime) {
       llvm::errs() << "Infer elapsed time=" << elapsedInferTime << "\n";
       ExecutionState &state = searcher->selectState();
       successorsPaths->erase(successorsPaths->begin(), 
                              successorsPaths->end());
       successorsPaths->insert(&state);
       ...
       if (tid < -1) {
       ...
       else if (tid == -1) { // main thread
          KInstruction *ki = state.pc;
          stepInstruction(state);
          llvm::errs() << "after step instruction for the main thread\n"; 
          ki->inst->dump(); 
          /* SYSREL extension */
          ...
          state.valueMap.clear();
          state.memobjMap.clear();
          executeInstruction(state, ki, &IP, NULL);
          updatePathWithValueMemObjectMaps(state, state.Path.size()-1);
          /* SYSREL extension */
          if (RecordValueFlow)
             generateValuedInstruction(state, ki); 
          // First check if any programming model related action 
          // to be executed
          if (singleSuccessor) {
             bool abort = false;
             ref<Expr> offendingAddress = Expr::createPointer(0);
          ...
          /* SYSREL extension */  
          llvm::errs() << "after execute instruction for the main thread\n";
          processTimers(&state, MaxInstructionTime);
          ...
          llvm::errs() << "scheduling thread " << tid << "\n"; 
          KInstruction *kia = state.threads[tid].pc;
          stepInstructionThread(state, tid);
          llvm::errs() << "after step instruction for thread " << tid << "..\n";
          kia->inst->dump();      
          state.valueMap.clear();
          state.memobjMap.clear();
          executeInstructionThread(state, kia, tid, &IP, NULL);
          updatePathWithValueMemObjectMaps(state, state.Path.size()-1);
          if (RecordValueFlow) 
             generateValuedInstruction(state, kia);
          llvm::errs() << "after execute instruction for thread " << tid << "..\n";
          processTimers(&state, MaxInstructionTime);
          checkMemoryUsage();       
          updateStates(&state);       
       }
       elapsedInferTime = statsTracker->elapsed() - inferStartTime;
   }  
   llvm::errs() << "Infer step end status: numstates=" << states.size() 
                << "executionHalted? " << haltExecution 
                << "numforks=" << stats::forks 
                << "elapsed vs max : " << elapsedInferTime << " vs " <<  MaxInferTime << "\n";
}


void Executor::exploreInstructionLevelScheduling(ExecutionState 
                                                      &initialState) {

   initRun(initialState);

   llvm::errs() << "Block level scheduling\n" 
                << "initialState " <<  &initialState << "\n";

   while (!states.empty() && !haltExecution) {
       ExecutionState &state = searcher->selectState();

       llvm::errs() << "chosen state " << &state << "\n";

       successorsPaths->erase(successorsPaths->begin(), 
                              successorsPaths->end());
       successorsPaths->insert(&state);
       int tid = -2;
       llvm::errs() << "Current thread " << state.rtid << "\n"; 
       if (state.isEnabled(state.rtid)) 
          tid = state.rtid;
       else {
          tid = state.scheduleAsync(memory);
       ...
       if (tid < -1) {
          ...
          llvm::errs() << "scheduling thread " << tid << "\n"; 
          KInstruction *kia = state.pc;
          dispatchAlternativeInstructions(state, kia, state.prevPC, &altschedules);
          llvm::errs() << "current inst " << (*(kia->inst)) << "\n";
          stepInstruction(state);
          llvm::errs() << "after step instruction for thread " << tid << "..\n";
          kia->inst->dump();      
          state.valueMap.clear();
...
void Executor::exploreBlockLevelScheduling(ExecutionState &initialState) {
   ...
   while (!states.empty() && !haltExecution) {
       ...
       if (tid < -1) {
          ...
          llvm::errs() << "scheduling thread " << tid << "\n"; 
          KInstruction *kia = state.pc;
          dispatchAlternativeBlocks(state, kia, state.prevPC, &altschedules);
          llvm::errs() << "current inst " << (*(kia->inst)) << "\n";
          stepInstruction(state);
          llvm::errs() << "after step instruction for thread " << tid << "..\n";
          kia->inst->dump();      
...
void Executor::exploreSelective(ExecutionState &initialState,
                     std::set<std::pair<Instruction*,std::string> > IP) {

   initRun(initialState);

   llvm::errs() << "initialState " << &initialState << "\n";
   double elapsedSelectTime = 0;
   double selectStartTime = statsTracker->elapsed();
   while (!states.empty() && !haltExecution && elapsedSelectTime < MaxSelectTime) {
       llvm::errs() << "Selective elapsed time=" << elapsedSelectTime << "\n";
       ExecutionState &state = searcher->selectState();
       ...
       if (tid < -1) {
          ...
          llvm::errs() << "scheduling thread " << tid << "\n"; 
          KInstruction *kia = state.pc;
          dispatchAlternativeSchedules(state, kia, state.prevPC, &IP, &altschedules);
          llvm::errs() << "current inst " << (*(kia->inst)) << "\n";
          stepInstruction(state);
          llvm::errs() << "after step instruction for thread " << tid << "..\n";
          kia->inst->dump();      
          state.valueMap.clear();
          state.memobjMap.clear();
          executeInstruction(state, kia, &IP, &altschedules);
          resetActiveInference(state);
          updatePathWithValueMemObjectMaps(state, state.Path.size()-1);
          if (RecordValueFlow) 
             generateValuedInstruction(state, kia);
          ...
          llvm::errs() << "scheduling thread " << tid << "\n"; 
          KInstruction *kia = state.threads[tid].pc;
          dispatchAlternativeSchedules(state, kia, state.threads[tid].prevPC, &IP, &altschedules);
          stepInstructionThread(state, tid);
          llvm::errs() << "after step instruction for thread " << tid << "..\n";
          kia->inst->dump();      
          state.valueMap.clear();
          state.memobjMap.clear();
          //std::vector<ExecutionState*> altschedules;
          executeInstructionThread(state, kia, tid, &IP, &altschedules);
          resetActiveInference(state);
          updatePathWithValueMemObjectMaps(state, state.Path.size()-1);
          if (RecordValueFlow) 
             generateValuedInstruction(state, kia);
       ...
       checkMemoryUsage();       
       updateStatesMT(&state, altschedules);       
       elapsedSelectTime = statsTracker->elapsed() - selectStartTime;
    }

}


void Executor::clearModuleConstants() {
  for (std::vector<KFunction*>::iterator it = kmodule->functions.begin(), 
         ie = kmodule->functions.end(); it != ie; ++it) {
    KFunction *kf = *it;
    for (unsigned i=0; i<kf->numInstructions; ++i) {
...
ExecutionState *Executor::reset() {
        searcher = 0;  
        clearModuleConstants();
        schedules.clear();
        states.clear();
        addedStates.clear();
        removedStates.clear(); 
        terminatedStates.clear();
        globalObjects.clear();
        globalAddresses.clear();
        globalAddressesRev.clear();
        memory = new MemoryManager(&arrayCache);
        KFunction *ekf = kmodule->functionMap[entryFunc];
        ExecutionState *secondRoundIS = new ExecutionState(ekf);
        if (!lazyInit) {
           for(unsigned i = 0, e = entryFunc->arg_size(); i != e; ++i) {
              ref<Expr> arge = entryFunctionArgs[i];
              bindArgument(ekf, i, *secondRoundIS, arge);
           }
        }
        else {
           // Lazy init args of the entryFunc
           bool abort = false;
           initArgsAsSymbolic(*secondRoundIS, entryFunc, abort);
           assert(!abort);
        }
        return secondRoundIS;
}

void Executor::initRun(ExecutionState &initialState) {
  if (!asyncMode) return;

  // creates issues with gep instructions..
  bindModuleConstants();
...
void Executor::run(ExecutionState &initialState) {
 ...
 if (asyncMode) { 
  double inferStart, inferEnd; 
  double dataFlowTotal = 0; 
  if (TargetFunctions != "")
     readTargetFunctions(TargetFunctions);
  else 
     assert(0 && "Please specify the file that containts the target functions using -target-function-list!\n"); 
  std::ofstream cf(ConcurrencyInfo.c_str(), std::ofstream::out);
  std::ofstream pf(InterleavingPointsInfo.c_str(), std::ofstream::out);
  CF = &cf;
  llvm::errs() << "scheduling mode=" << SchedulingMode << "\n"; 
  cf << "scheduling mode=" << SchedulingMode << "\n"; 
  //cf << "0: selective, 1: block level, 2: instruction level, 3: assertion based \n";
  cf << "0: selective, 1: block level, 2: instruction level \n";
  cf << "number of steps for selective mode=" << NumExploreSelSteps << "\n"; 
  if (asyncMode && SchedulingMode == 3) { 
     assertionBasedChecking = true;
      //  Assertion-based checking
     exploreAssertionBased(initialState);
     return; 
  }
  else if (asyncMode && SchedulingMode == 0) {
     std::set<std::pair<Instruction*,std::string> > IP, IP2;
     inferStart = statsTracker->elapsed();
     exploreInfer(initialState, IP);
     inferEnd = statsTracker->elapsed();
     cf << "Infer Time=" << (inferEnd - inferStart) << "\n";
     double dfs = statsTracker->elapsed();
     std::vector<std::set<std::pair<Instruction*, 
                                    std::string> > *> Part1, Part2;
     for (const auto &state : states) {
         llvm::errs() << "Computing interleaving points for active state " << &state << "\n";
         state->computeInterleavingPoints(IP, Part1);
     }
     for (const auto &state : terminatedStates) {
         llvm::errs() << "Computing interleaving points for terminated state " << &state << "\n";
         state->computeInterleavingPoints(IP, Part1);
     }
     double dfe = statsTracker->elapsed();
     cf << "DataFlow Analysis Time=" << (dfe-dfs) << "\n";
     cf << "Errors in inference step: " << numErrors << "\n";
     cf << "Elapsed time=" << statsTracker->elapsed() << "\n";
     numErrors = 0;
     cf << "Moving to selective exploration with " 
                  << IP.size() << " inference points:\n";
     llvm::errs() << "Moving to selective exploration with ";
     if (PartitionInferencePoints)
        llvm::errs() << Part1.size() << " partitions and "; 
     llvm::errs() << IP.size() << " inference points:\n";
     for(auto ipp : IP) {
        llvm::errs() << "inst=" << (*ipp.first) << " context= " << ipp.second << "\n";
     }
     llvm::errs() << "Number of unique schedules: " << schedules.size() << "\n";
     cf << "Number of unique schedules: " << schedules.size() << "\n";
     ...
     cf << "Number of paths " << (states.size() + terminatedStates.size()) << "\n";
     cf.flush();
     delete searcher;
     for(int numSteps = 0; numSteps < NumExploreSelSteps; numSteps++) 
     {
        std::vector<std::set<std::pair<Instruction*, 
        ...
        unsigned numinf=0;
        for(unsigned pn=0; pn<Partitions.size(); pn++)
           numinf += Partitions[pn]->size();
        cf << "Selective step " << numSteps << " with " 
           << "total " << numinf << " inferences ";
        if (PartitionInferencePoints)
           cf << Partitions.size() << " partitions\n"; 
        else cf << "\n";  
       for(unsigned pn=0; pn<Partitions.size(); pn++)
       {  
        if (PartitionInferencePoints)
           cf << "Partition " << pn << " size=" 
              << Partitions[pn]->size() << "\n"; 
        cf.flush();
        llvm::errs() << "Partition " << pn << " size=" 
           << Partitions[pn]->size() << "\n"; 
        pf << "Partition " << pn << " size=" << Partitions[pn]->size() << "\n";
        for(auto ipp : (*Partitions[pn])) {
           pf << "inst=";
           std::string type_str;
           llvm::raw_string_ostream rso(type_str);
           ipp.first->print(rso);
           pf << rso.str() << " context= " << ipp.second << "\n";
           llvm::errs() << "inst=" << (*ipp.first) 
                        << " context= " << ipp.second << "\n";
        }
        pf.flush();
        ExecutionState *secondRoundIS = reset();
        initializeGlobals(*secondRoundIS);
        processTree = new PTree(secondRoundIS);
        secondRoundIS->ptreeNode = processTree->root;
        if (pathWriter) 
           secondRoundIS->pathOS = pathWriter->open();
        if (symPathWriter) 
           secondRoundIS->symPathOS = symPathWriter->open();
        if (statsTracker)
           statsTracker->framePushed(*secondRoundIS, 0);
        double ses = statsTracker->elapsed();
        exploreSelective(*secondRoundIS, *Partitions[pn]);
        double see = statsTracker->elapsed();
        cf << "Selective " << numSteps << " partition " 
           << numSteps << " Time=" << (see - ses) << "\n";
        double dfs = statsTracker->elapsed();
        unsigned numSt = 0;
        for(const auto &state : terminatedStates) {
           if (numSt++ > MaxSelStates) break;
           llvm::errs() << "Computing interleaving points II for terminated state " << &state << "\n";
           state->dumpMemobjects(); 
           state->computeInterleavingPoints(IP2, Part2);
        }
        for(const auto &state : states) {
           if (numSt++ > MaxSelStates) break;
           llvm::errs() << "Computing interleaving points II for active state " << &state << "\n";
           state->dumpMemobjects(); 
           state->computeInterleavingPoints(IP2, Part2);
        }
        double dfe = statsTracker->elapsed();
        cf << "DataFlow Analysis Time=" << (dfe-dfs) << "\n";
        cf << "Errors in selective step " << numSteps 
           << ": " << numErrors << "\n";
        cf << "Elapsed time=" << statsTracker->elapsed() << "\n";
        numErrors = 0;
        llvm::errs() << "Ending selective exploration with " 
                  << IP2.size() << " inference points:\n";
        cf << "Ending selective exploration with " 
                  << IP2.size() << " inference points:";
        if (PartitionInferencePoints)
           cf << " and " << Part2.size() << " partitions\n";      
        else cf << "\n";
        for(auto ipp : IP2) {
           llvm::errs() << "inst=" << (*ipp.first) << " context= " << ipp.second << "\n";
        }
     ...
     if (TryIndivInferencePoints) {
        unsigned ni = 0;
        llvm::errs() << "Applying " << IP.size() << " inference points one at a time!\n";
        for(auto ipp : IP) {
           llvm::errs() << "inference point: " << ni << "inst=" 
                      << (*ipp.first) << " context= " << ipp.second << "\n";
         std::set<std::pair<Instruction*,std::string> > IPInd;
         IPInd.insert(ipp);
         ExecutionState *secondRoundIS = reset();
         initializeGlobals(*secondRoundIS);
         processTree = new PTree(secondRoundIS);
         ...
         if (statsTracker)
            statsTracker->framePushed(*secondRoundIS, 0);
         exploreSelective(*secondRoundIS, IPInd);         
         cf << "Errors in selective step for inference point " << ni 
            << ": " << numErrors << "\n";
         cf << "Elapsed time=" << statsTracker->elapsed() << "\n";
         cf.flush();
         ni++;   
     ...
     if (TryInferencePointPairs) {
        unsigned ni = 0;
        llvm::errs() << "Applying " << IP.size()*IP.size() 
                     << " inference points pairs!\n";
        std::set<std::pair<Instruction*,std::string> > IPCopy = IP;
        for(auto ipp1 : IP) {
         for(auto ipp2 : IPCopy) {
            if (ipp1.first == ipp2.first) continue;
         std::set<std::pair<Instruction*,std::string> > IPPair;
         IPPair.insert(ipp1);
         IPPair.insert(ipp2);
         ExecutionState *secondRoundIS = reset();
         initializeGlobals(*secondRoundIS);
        ...
        if (statsTracker)
            statsTracker->framePushed(*secondRoundIS, 0);
         exploreSelective(*secondRoundIS, IPPair);         
         if (numErrors > 0) {
            cf << "Errors in selective step for inference point pairs" << ni 
             << ": " << numErrors << "\n";
            cf.flush();
            break;
         }
  ...
  else if (asyncMode && SchedulingMode == 1) {
     exploreBlockLevelScheduling(initialState);
     llvm::errs() << "Number of unique schedules: " << schedules.size() << "\n";
     for(auto s : schedules) {
        llvm::errs() << "\n=====================================================\n";
        llvm::errs() << "Thread schedule:\n";
     ...
     cf << "#Errors in block level scheduling mode: " << numErrors << "\n";
     cf << "Number of unique schedules: " << schedules.size() << "\n";
     cf << "Number of paths " << (states.size() + terminatedStates.size()) << "\n";
     delete searcher;
     return;
  }
  else if (asyncMode && SchedulingMode == 2) {
     exploreInstructionLevelScheduling(initialState);
     llvm::errs() << "Number of unique schedules: " << schedules.size() << "\n";
     for(auto s : schedules) {
        llvm::errs() << "\n==================================================\n";
        llvm::errs() << "Thread schedule:\n";
     ...
     cf << "#Errors in instruction level scheduling mode: " << numErrors << "\n";
     cf << "Number of unique schedules: " << schedules.size() << "\n";
     cf << "Number of paths " << (states.size() + terminatedStates.size()) << "\n";
     delete searcher;
     return;  
  ...
  if (PrioritizePaths && PrioritizationStatsFile != "") {
     prf = new std::ofstream(PrioritizationStatsFile.c_str());   
     initalizeForPriorityMode(); 
  }
  ...
  while (!states.empty() && !haltExecution) {
    ...
    /*if (PrioritizePaths) {
       (*prf) << "Scheduling a state " << &state << " at pc=" << state.pc->getSourceLocation() 
             << " while num states= " << states.size() << ":\n";
       for(auto s : states)
          (*prf) << "\tstate=" << s << "\n";   
       (*prf) << "stashed=" << stashedStates.size() << "\n";
       for(auto s : stashedStates) 
          (*prf) << "\tstashed state=" << s << "\n";  
    }*/
    ...
    /* SYSREL */
      ...
      ExprPPrinter::printConstraints(llvm::errs(), state.constraints);
      executeInstruction(state, ki);
      checkTargetUpdatePrioritized(state, ki);
      if (RecordValueFlow)
         generateValuedInstruction(state, ki);
      /* SYSREL extension */
      // First check if any programming model related action to be executed
      ...
      processTimers(&state, MaxInstructionTime);
      checkMemoryUsage();
      if (PrioritizePaths && !stashedStates.empty()) {
         // keep the invariants about states and removedstates intact
         std::set<ExecutionState*> rs;
         for(auto s : stashedStates) {
            llvm::errs() << "stashed state " << s << "\n";
            for (std::vector<ExecutionState *>::iterator it = removedStates.begin(),
                    ie = removedStates.end(); it != ie; it++) {
               if (*it == s) {
                  states.insert(s);
                  // TODO: MAke it work for searchers other than DFSSearcher!
                  searcher->unstashState(s);
                  rs.insert(s);
                  break;
               }
            }
         ...
         for(auto r : rs) {
            stashedStates.erase(r);
         }
      }
      /* (*prf) << " Before update num states= " << states.size() << ":\n";
       for(auto s : states)
          (*prf) << "\tstate=" << s << "\n";   
       (*prf) << "stashed=" << stashedStates.size() << "\n";
       for(auto s : stashedStates) 
      ...
      */ 
      updateStates(&state);
      /* (*prf) << " After update num states= " << states.size() << ":\n";
       for(auto s : states)
          (*prf) << "\tstate=" << s << "\n";   
       (*prf) << "stashed=" << stashedStates.size() << "\n";
       for(auto s : stashedStates) 
          (*prf) << "\tstashed state=" << s << "\n";  
      */
      if (states.empty() && PrioritizePaths && !stashedStates.empty()) {
         llvm::errs() << "Continuing paused states num: " << stashedStates.size() << "\n";
         (*prf) << "Continuing paused states num: " << stashedStates.size() << "\n";
         prf->flush();
         for(auto s : stashedStates) {
            states.insert(s); 
            searcher->unstashState(s);
      ...
      /*      
      (*prf) << " After possible update num states= " << states.size() << ":\n";
      for(auto s : states)
          (*prf) << "\tstate=" << s << "\n";   
      (*prf) << "stashed=" << stashedStates.size() << "\n";
      for(auto s : stashedStates) 
...
void Executor::terminateState(ExecutionState &state) {
  ...
  state.check();
  if (asyncMode)
     state.completePath(state.rtid);  

  llvm::errs() << "Path ending:\n"; 
  state.dumpStateWithThreads();
  schedules.insert(state.getThreadSchedule());
  /* SYSREL extension */
  ...
  if (it==addedStates.end()) {
    ...
    llvm::errs() << "state " << &state << " is being removed\n";
...
void Executor::terminateStateOnElimination(ExecutionState &state) {
  llvm::errs() << "Terminating state via pruning based on weakest-precondition\n";
  if (asyncMode) {
     numEliminated++;
     (*CF) << "State eliminated at time=" << statsTracker->elapsed() << "\n";
     CF->flush(); 
  }
  terminateState(state);
}

void Executor::terminateStateOnCancel(ExecutionState &state) {
  llvm::errs() << "Terminating state on cancel!\n";
  restoreChildCount(state);
  state.cancelled = true;
  terminateState(state);
}

void Executor::terminateStateOnExit(ExecutionState &state) {
  llvm::errs() << "Terminating normally..\n";
  if (!OnlyOutputStatesCoveringNew || state.coveredNew || 
      (AlwaysOutputSeeds && seedMap.count(&state))) {
     if (recordStackTraces) {
        std::string straces = getLongestStackTraces(state, stackTraceCount);
...
void Executor::terminateStateOnError(ExecutionState &state,
  ...
  if (asyncMode) {
     numErrors++;
     (*CF) << "Error detected at time=" << statsTracker->elapsed() << "\n";
     CF->flush(); 
     state.completePath(state.rtid); 
  }
  ...
  if (EmitAllErrors || (state.hasLCM() ? emittedErrorsAlt.insert(std::make_tuple(lastInst, message, state.getLCMState())).second : 
    ...
    msg << "Error: " << message << "\n";
    if (asyncMode) {
       msg << "Error detected at time=" << statsTracker->elapsed() << "\n";
       msg << "Main thread:\n";
       if (state.terminated) 
             msg << "terminated.\n";
       else if (!state.isEnabled(-1)) {
            msg << "blocked.\n";
       }
       else {
          msg << "active.\n";
    ...
    if (asyncMode) {
       ...
       msg << "\n=====================================================\n";
       msg << "Thread schedule:\n";
       msg << state.getThreadSchedule() << "\n"; 
       msg << "\n=====================================================\n";
       for(unsigned int tid=0; tid<state.threads.size(); tid++) {
          Async &a = state.threads[tid];
          msg << "Thread " << a.tid << "\n";
          msg << "Main function: " << a.function->getName();
          msg << "Status: ";
          if (a.terminated) 
             msg << "terminated.\n";
          else if (!state.isEnabled(tid)) {
            msg << "blocked.\n";
          }
          else {
            msg << "active.\n";
          ...
          if (tii->file != "") {
              msg << "File: " << tii->file << "\n";
              msg << "Line: " << tii->line << "\n";
              msg << "assembly.ll line: " << tii->assemblyLine << "\n";
          }
          msg << "Stack: \n";
          state.dumpStackThread(msg, tid, false);
          msg << "\n=====================================================\n";
       }
    }
...
void Executor::callExternalFunction(ExecutionState &state,
  ...
  if (specialFunctionHandler->handle(state, function, target, arguments)) {
     if (APIHandler::isIgnore(function->getName())) {
 ...
 /* SYSREL EXTENSION */
  ...
  if (!success) {
    ...
    if (progModel) {
     ...
     // check if the function is modeled by another function
     if (functionModeledBy.find(removeDotSuffix(function->getName())) != 
                                       functionModeledBy.end()) {
        llvm::errs() << "WARNING: Using " 
                     << functionModeledBy[removeDotSuffix(function->getName())] 
	 ...
	 terminateStateOnError(state, "Memory error", Ptr, NULL, 
       ...
       if (resHandled && APIHandler::isIgnore(function->getName())) {
          llvm::errs() << "Should ignored function " << function->getName() << " be havoced?\n";
          if (havocArgs.find(function->getName()) != havocArgs.end()) {
             llvm::errs() << "will havoc the sepcified args for " 
                       << function->getName() << "\n";
             // havoc those arguments that are specified
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    if (asyncMode && state.rtid > -1)
       allocSite = state.threads[state.rtid].prevPC->inst;
    ...
    updatePathWithValueMemObjectMaps(state,state.Path.size()-1);
    if (!isLocal) {
       state.globals.insert(CE2->getZExtValue());
       llvm::errs() << "recording global obj " << CE2->getZExtValue() << "\n";
    }
    /* SYSREL extension */
   ...
   if (lazyInit) {
    ...
    if (!mo) {
      if (asyncMode) {
         llvm::errs() << "Thread " << state.rtid 
                      << ", returning NULL as the result of alloc!\n";
         if (state.rtid == -1)
            bindLocal(target, state, 
                ConstantExpr::alloc(0, Context::get().getPointerWidth()));
         else   
            bindLocalThread(target, state, 
                ConstantExpr::alloc(0, Context::get().getPointerWidth()), state.rtid);
      ...
      //if (state.PathValueObject.find(cindex) != state.PathValueObject.end())
      //   prev = state.PathValueObject[cindex];
      //prev[-1] = ConstantExpr::alloc(0, Context::get().getPointerWidth());
      //state.PathValueObject[cindex] = prev;
      int cindex = state.stack.back().lastCallIndex;
      if (cindex == state.Path.size() - 1)
         state.valueMap[-1] = ConstantExpr::alloc(0, Context::get().getPointerWidth());
      //llvm::errs() << "Storing return value of an alloc operation " << prev[-1] 
        //           << "for inst with index " << cindex 
          //         << " address=" << &state.PathValueObject <<  "\n";
      //for(auto m : state.PathValueObject[cindex])
        // if (m.first == -1)
          //  llvm::errs() << "Reading back " << m.second << "\n"; 
    } else {
      ...
      if (asyncMode) {
         if (state.rtid == -1) {
            ...
            if (getDestCellThread(state, target, state.rtid).value.get())
               llvm::errs() << "address to be dest: " << getDestCellThread(state, target, state.rtid).value << "\n";
            }
      }
      else if (getDestCell(state, target).value.get())
         llvm::errs() << "address to be dest: " << getDestCell(state, target).value << "\n";
      ...
      if (asyncMode) {
         if (state.rtid == -1)
            ...
            bindLocalThread(target, state, mo->getBaseExpr(), state.rtid);
      }
      else 
         bindLocal(target, state, mo->getBaseExpr());
      ...
      if (cindex == state.Path.size() - 1) {
         if (state.rtid != -1)
            state.valueMap[state.rtid] = mo->getBaseExpr();
         else 
            state.valueMap[-1] = mo->getBaseExpr();
      }
      //std::map<int, ref<Expr> > prev;
      //if (state.PathValueObject.find(cindex) != state.PathValueObject.end())
      //   prev = state.PathValueObject[cindex];
      //prev[-1] = mo->getBaseExpr();
      //state.PathValueObject[cindex] = prev;
      //llvm::errs() << "Storing return value of an alloc operation " << prev[-1]
      //             << "for inst with index " << cindex 
      //             << " address=" << &state.PathValueObject << "\n";
      //for(auto m : state.PathValueObject[cindex])
        //  llvm::errs() << "Reading back " << m.first << "->" << m.second << "\n"; 
    ...
    if (fixedSize.second) { 
      ...
      if (res) {
        ...
        if (state.PathValueObject.find(cindex) != state.PathValueObject.end())
           prev = state.PathValueObject[cindex];
        if (state.rtid != -1)
           prev[state.rtid] = ConstantExpr::alloc(0, Context::get().getPointerWidth());
        else 
           prev[-1] = ConstantExpr::alloc(0, Context::get().getPointerWidth());
        state.PathValueObject[cindex] = prev;
...
void Executor::executeFree(ExecutionState &state,
  ...
  if (state.rtid == -1)
     ...
     llvm::errs() << "Executing free " << address << " state=" << &state << " from thread " << state.rtid <<"\n"; 
    ...
    if (target) {
      if (asyncMode) {
         if (state.rtid == -1)
            ...
            bindLocalThread(target, *zeroPointer.first, Expr::createPointer(0), 
                            state.rtid);
      }
      else 
         bindLocal(target, *zeroPointer.first, Expr::createPointer(0));
  ...
  if (zeroPointer.second) { // address != 0
    ...
    for (Executor::ExactResolutionList::iterator it = rl.begin(), 
      ...
      if (mo->isLocal) {
        ...
        if (target) {
           if (asyncMode) {
              if (state.rtid == -1)
                 ...
                 bindLocalThread(target, *it->second, Expr::createPointer(0),
                           state.rtid);
           }
           else
               bindLocal(target, *it->second, Expr::createPointer(0));
        ...
        ref<Expr> ma = mo->getBaseExpr();
        ConstantExpr *CE2 = dyn_cast<ConstantExpr>(ma);
        it->second->memobjMap[ma] = CE2->getZExtValue();
        updatePathWithValueMemObjectMaps(*it->second,
                                  it->second->Path.size()-1);
...
bool Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (success) {
   ...
   state.lastAccessedAddr = address;
   state.lastAccessedMem = mo;
      ...
      if (isWrite) {
        if (os->readOnly) {
          ...
          if (WriteMap.find(mo) != WriteMap.end())
             wms = WriteMap[mo];
          wms.insert(std::make_pair(-1, lastStore));
          WriteMap[mo] = wms;*/
        }          
      } else {
        ref<Expr> result = os->read(offset, type);
        llvm::errs() << "read " << result << " from "  
                     << address << " at offset " 
                     << offset << " size " << type << "\n"; 
          ...
          if (ace)
             ...
             ref<Expr> bexp = mo->getBaseExpr();
             ConstantExpr *ace = dyn_cast<ConstantExpr>(bexp);
             state.memobjMap[result] = ace->getZExtValue();
          //}
...
void Executor::runFunctionAsMain(Function *f,
  ...
  llvm::errs() << "num args " << argc << " num envc " << envc << "\n";
 ...
 /* SYSREL extension end */
  ...
  if (ai!=ae) {
    ...
    if (++ai!=ae) {
      ...
      ref<Expr> argvMOB = argvMO->getBaseExpr();
      argvMOA = dyn_cast<ConstantExpr>(argvMOB);
  ...
  //if (argvMOA)
  //   state->globals.insert(argvMOA->getZExtValue());
 ...
 if (!lazyInit  || UseConcreteArgs) {
  for (unsigned i = 0, e = f->arg_size(); i != e; ++i) {
    llvm::errs() << "Binding arg " << i << " with \n";
    llvm::errs() << arguments[i] << "\n";
    bindArgument(kf, i, *state, arguments[i]);
    entryFunctionArgs.push_back(arguments[i]);
  }
 }
 /* SYSREL extension end */
  ...
  if (argvMO) {
    ...
    for (int i=0; i<argc+1+envc+1+1; i++) {
      if (i==argc || i>=argc+1+envc) {
        ...
        ref<Expr> argB = arg->getBaseExpr();
        ConstantExpr *argA = dyn_cast<ConstantExpr>(argB);
        state->globals.insert(argA->getZExtValue());


        ObjectState *os = bindObjectInState(*state, arg, false);
        for (j=0; j<len+1; j++) {
          os->write8(j, s[j]);
          //llvm:errs() << "Writing arg char " << s[j] << "\n";
        }
...
Expr::Width unsized(llvm::Type *type) {
  if (getTypeName(type) == "i8")
     return 8;
  if (getTypeName(type) == "i16")
     return 16;
  if (getTypeName(type) == "i32")
     return 32;
  if (getTypeName(type) == "i64")
     return 64;
...
Expr::Width Executor::getWidthForLLVMType(llvm::Type *type) const {
  llvm::errs() << "Type " << getTypeName(type) << "\n";
  if (!type->isSized())
     return unsized(type);
...
size_t Executor::getAllocationAlignment(const llvm::Value *allocSite) const {
  ...
  if (const GlobalValue *GV = dyn_cast<GlobalValue>(allocSite)) {
    ...
    llvm::errs() << "alloc site: " << (*allocSite) << "\n";
