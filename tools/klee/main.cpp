...
namespace {
  ...
  cl::opt<bool> 
  TurnOffNormalExec("turnoff-normal-tests",
              cl::desc("Turnoff test case generation for normal or early\n"),
              cl::init(true));
...
void KleeHandler::processTestCase(const ExecutionState &state,
                                  const char *errorMessage,
                                  const char *errorSuffix) {
  if (asyncMode && TurnOffNormalExec && 
                  (!errorSuffix || strcmp(errorSuffix,"early") == 0))
     return;
  if (!NoOutput) {
