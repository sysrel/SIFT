...
public:
 ...
 void propagateWP(WPState *wps, ref<Expr> wp);

 void setWPState(ExecutionState &prevstate, ExecutionState &nextstate, 
...
/* SYSREL extension */
  ...
  std::set<ExecutionState*> stashedStates;
...
/* SYSREL extension */
  void executeInstruction(ExecutionState &state, KInstruction *ki, 
          std::set<std::pair<Instruction*,std::string> > *IP=NULL, 
                       std::vector<ExecutionState*> *altsch=NULL);

  /* SYSREL extension */
  void executeInstructionThread(ExecutionState &state, KInstruction *ki, 
          int tid, std::set<std::pair<Instruction*,std::string> > *IP=NULL, 
                               std::vector<ExecutionState*> *altsch=NULL);

  void clearModuleConstants();  

  ExecutionState *reset();

  void initRun(ExecutionState &initialState); 

  void exploreAssertionBased(ExecutionState &initialState);

  void exploreInfer(ExecutionState &initialState,
                      std::set<std::pair<Instruction*,std::string> >&IP);

  void exploreSelective(ExecutionState &initialState,
                     std::set<std::pair<Instruction*,std::string> > IP) ;

  void exploreBlockLevelScheduling(ExecutionState &initialState);

  void exploreInstructionLevelScheduling(ExecutionState &initialState);
  ...
  void dispatchAlternativeBlocks(ExecutionState &state, 
                                            KInstruction *ki, 
                                            KInstruction *pki, 
                          std::vector<ExecutionState*> *alt);

  void dispatchAlternativeInstructions(ExecutionState &state, 
                                            KInstruction *ki, 
                                            KInstruction *pki, 
                          std::vector<ExecutionState*> *alt);

  void dispatchAlternativeSchedules(ExecutionState &state, KInstruction *ki,
                    KInstruction *pki,
                    std::set<std::pair<Instruction*,std::string> > *IP, 
                                  std::vector<ExecutionState*> *alt);
  ...
  void updateStatesMT(ExecutionState *current,
  ...
  /* SYSREL extension */   
  bool handledModeledCall(ExecutionState &state, 
                           KInstruction *ki,
                           Function *f,
                           std::vector< ref<Expr> > &arguments);

  bool handledModeledCallThread(ExecutionState &state, 
                           KInstruction *ki,
                           Function *f,
                           std::vector< ref<Expr> > &arguments, int tid);
  ...
  void executeMemoryOperationThread(ExecutionState &state,
                                      ...
                                      ref<Expr> address,
                                      ref<Expr> value /* undef if read */,
					      KInstruction *target /* undef if write */, int tid, bool loadstore = false) ;
...
private:
 ...
 public:
  ...
  /* SYSREL extension */
  bool isInScope(ExecutionState &state, KInstruction *target, KInstruction *br);
  void checkTargetUpdatePrioritized(ExecutionState &state, KInstruction *ki);
  bool isCallATarget(ExecutionState &state, KInstruction *ki);
  bool isAReceiveCall(ExecutionState &state, KInstruction *ki);
  bool isASendCall(ExecutionState &state, KInstruction *ki);
  bool isATargetInstruction(ExecutionState &state, KInstruction *ki);
  /* SYSREL extension */
...
private:
  ...
  /* SYSREL extension */
  // prune/eliminate state on some criteria, e.g., won't lead to assertion failure
  void terminateStateOnElimination(ExecutionState &state);
  // cancel state (that should not have been created)
  void terminateStateOnCancel(ExecutionState &state);
  // call exit handler and terminate state
