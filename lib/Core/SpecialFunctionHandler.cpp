...
#define addDNR(name, handler) { name, \
  ...
  add("__VERIFIER_assume", handleAssume, true),
  /* SYSREL EXTENSION */
...
void SpecialFunctionHandler::handlePthreadCreate(ExecutionState &state,
                                                 KInstruction *target,
                                     std::vector<ref<Expr> > &arguments) {
    //std::string fname = readStringAtAddress(state, arguments[2]);
    ConstantExpr *CE = dyn_cast<ConstantExpr>(arguments[2]);
    assert(CE && "Symbolic function address expression not handled!\n");
    Function *func = (Function*)CE->getZExtValue();
    state.createThread(target, func, arguments[3], arguments[0]);
}
...
void SpecialFunctionHandler::handlePthreadJoin(ExecutionState &state,
    ...
    assert(CE && "Symbolic expression for thread id not supported!\n");
    llvm::errs() << "pthread join id val: " << arguments[0] << "\n";
    long mtid = CE->getZExtValue();
    assert(state.threadIdMap.find(mtid) != state.threadIdMap.end());
    int id = state.threadIdMap[mtid];
    state.threadJoinsThread(target, state.rtid, id);
}
...
void SpecialFunctionHandler::handlePthreadMutexInit(ExecutionState &state,
                                                 KInstruction *target,
                                     std::vector<ref<Expr> > &arguments) {
  state.initLock(target, arguments[0]);
}

/*void SpecialFunctionHandler::handleVerifierAssume(ExecutionState &state,
                                                 KInstruction *target,
                                     std::vector<ref<Expr> > &arguments) {
  llvm::errs() << "Adding the assumption " << arguments[0] << "\n";
  ConstantExpr *CE = dyn_cast<ConstantExpr>(arguments[0]);
  if (CE) {
     if (CE->getZExtValue() == 1)
        state.addConstraint(ConstantExpr::create(1,Expr::Bool));
     else if (CE->getZExtValue() == 0)
        state.addConstraint(ConstantExpr::create(0,Expr::Bool));
     else assert(false && "Adding a nonbool assumption through __VERIFIER_assume!\n");
  } 
  else state.addConstraint(ConstantExpr::create(0,Expr::Bool));
...
bool APIHandler::handle(ExecutionState &state, 
  ...
  if (!branching || !symbolizeRetValueOK) {
     if (branching && !symbolizeRetValueOK)
        llvm::errs() << "WARNING: api both branching and requesting not to symbolize the return value!\n";

     if (symbolizeRetValueOK) {
        llvm::errs() << "symbolizing ret value in handler for function " << function->getName() << "\n";
        if (asyncMode && state.rtid != -1) {
           ((Executor*)(theInterpreter))->symbolizeReturnValueThread(state, target, 
                                               function, abort, state.rtid);
        } 
        else ((Executor*)(theInterpreter))->symbolizeReturnValue(state, target, function, abort);
        if (abort) return false;
     }
   ...
   llvm::errs() << "Branching semantics, current path condition:\n";
   Function *f = kmoduleExt->module->getFunction(fname);
   assert(f);
   const MemoryObject *srt = NULL;
   if (asyncMode && state.rtid != -1)
      srt = ((Executor*)(theInterpreter))->symbolizeReturnValueThread(state, target, 
                                      function, abort, state.rtid);
   else
      srt = ((Executor*)(theInterpreter))->symbolizeReturnValue(state, target, function, abort);
   if (abort) return false;
   bool asuccess;
   ObjectPair op;
   if (srt) { 
